...
| L2 | System audio loopback | Play YouTube/browser audio — must appear on **System** channel only (not Mic). Zoom/Meet also valid (see `m13-live-pipeline-checklist.md` §A). **Do not mark PASS without device evidence.** | ☐ — retest using [`L2-loopback-retest-checklist.md`](./L2-loopback-retest-checklist.md) after pulse-first fix |
